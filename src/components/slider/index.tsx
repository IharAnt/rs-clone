    ...
    <div className="h-44 flex flex-col items-center justify-between">
      <div className="flex items-center">
        ...
        <div
          className={`duration-200 ${
            offset === MAX_OFFSET ? 'opacity-20' : 'opacity-50 cursor-pointer hover:opacity-100'
          }`}
      ...
      <div className="w-20 flex justify-between">
        {SLIDER_DATA.map((item, index) => {
          return (
            <div
              id={String(index * WIDTH_SLIDER)}
              key={index}
              className={`duration-200 w-4 h-4 rounded-full bg-indigo-800
                            ${
                              offset === -index * WIDTH_SLIDER
                                ? 'opacity-100'
                                : 'opacity-30 cursor-pointer hover:opacity-100'
                            }`}
